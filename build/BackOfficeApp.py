    ...
    def update_occupancy_data(self):
        try:
            ...
            for trimester, (start_month, end_month) in trimesters.items():
                ...
                # Total days per room type
                total_days *= 24  # Total hours in the trimester

                for tipo_camera in camere_data:
                    for camere_info in tipo_camera.values():
                        num_camere = len(camere_info[0].keys())
                ...
                if total_days > 0:
                    occupancy_rate = (occupied_days / (total_days * num_camere)) * 100
                else:
                    occupancy_rate = 0
